...
export default function Login() {
  ...
  const googleLogin = useGoogleLogin({
    flow: 'auth-code',
    ux_mode: 'redirect',
    redirect_uri: `${import.meta.env.VITE_FE_URL}/auth/google/callback`, 
  });

  const handleFacebookLogin = () => {
  const appId = import.meta.env.VITE_FACEBOOK_APP_ID;
  const redirectUri = `${import.meta.env.VITE_FE_URL}/auth/facebook/callback`;
  const state = "random_string_for_security";

  const fbAuthUrl = `https://www.facebook.com/v18.0/dialog/oauth?client_id=${appId}&redirect_uri=${redirectUri}&state=${state}&scope=email,public_profile`;
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-white/10 backdrop-blur-2xl border border-white/10 p-8 rounded-[2.5rem] shadow-2xl">  
          ...
          <button 
            type="button"
            onClick={() => googleLogin()} 
            className="w-full bg-white/5 hover:bg-white/10 border border-white/10 text-white py-3.5 rounded-2xl font-bold text-[10px] uppercase tracking-wider transition-all flex items-center justify-center gap-2 active:scale-95 shadow-sm"
          >
